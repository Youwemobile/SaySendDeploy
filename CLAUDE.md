...
- **`components/infra/`** — cluster-level infra chart: HAProxy Ingress (routes `/docs`, `/auth`, `/transcription`, `/translation` to the corresponding Services) and a Zalando `postgresql` operator CR (`acid.zalan.do/v1`).
